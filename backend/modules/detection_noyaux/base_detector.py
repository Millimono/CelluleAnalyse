...
class Noyau:
    ...
    elongation:          float          # rapport grand axe / petit axe
    intensite_adn:       float          # intensité moyenne canal ADN
    intensite_composite: float          # intensité moyenne composite (3 canaux)
...
class BaseDetector(ABC):
    ...
    Usage :
        ...
        resultat = detector.detect(image_adn_2d, image_composite_2d)
    """
    ...
    def detect(self, image: np.ndarray, image_composite: np.ndarray = None) -> ResultatDetection:
        ...
        Args:
            image           : array numpy 2D (Y, X) canal ADN en uint16
